...
const Search = () => {
  ...
  const { data: productList, run: getProductList } = useRequest(
    (data) => {
      setCurrentProductPage(data.pageNum || 1);
      return getSearchProductList(data);
    },
    {
      manual: true,
    },
  ...
  const { data: solutionList, run: getSolutionList } = useRequest(
    (data) => {
      setCurrentSolutionPage(data.pageNum || 1);
      return getSearchSolutionList(data);
    },
    {
      manual: true,
    },
  ...
  //   新闻分页数据
  const { data: newsList, run: getNewsList } = useRequest(
    (data) => {
      setCurrentNewsPage(data.pageNum || 1);
      return getSearchNewsList(data);
    },
    {
      manual: true,
    },
  ...
  const { data: downloadList, run: getDownloadList } = useRequest(
    (data) => {
      setCurrentDownloadPage(data.pageNum || 1);
